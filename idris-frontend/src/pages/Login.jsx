...
const Login = () => {
  ...
  const { loginCustomer, navigate, backendUrl } = useContext(ShopContext);
    ...
    try {
      if (currentState === 'Forgot Password') {
        ...
        if (response.data.success) {
          ...
          toast.success('Reset token created. Enter your new password.');
        } else {
          toast.error(response.data.message || 'Could not start password reset');
        }
        ...
        if (response.data.success) {
          loginCustomer(response.data.token, response.data.user);
      ...
      <button className='bg-black text-white font-light px-8 py-2 mt-4'>
        {
          ...
          currentState === 'Forgot Password' ? 'Get Reset Token' :
          'Reset Password'
        }
      </button>
